...
public class Phase1
{
	...
	public static void main(String[] args)
	{
		...
		Controller.highCardGame = new Controller.CardGameFramework(
	...
	static class Controller
	{
		...
		private static String[] playerNames = { "Computer", "You" };
		...
		{
			myCardTable = new CardView.CardTable("High Card", NUM_CARDS_PER_HAND,
					NUM_PLAYERS);
		...
		private static void startTheGame()
		{
			...
			for (int i = 0; i < NUM_CARDS_PER_HAND; i++)
			{
				...
				humanLabels[i] = new JLabel(CardView.GUICard
						.getIcon((highCardGame.getHand(1).inspectCard(i))));
		...
		private static int computerChooseCard()
		{
			...
			if (cardPlayed[1] != null)
			{
				...
				CardModel.Card[] ascHand = new CardModel.Card[highCardGame
						.getHand(0).getNumOfCards()];
				...
				for (int a = 0; a < ascHand.length; a++)
				{
					System.out.println(ascHand[a].toString() + " -> rank "
							+ CardModel.Card.getSortRanking(ascHand[a]));
				}
				...
				for (int x = 0; x < ascHand.length; x++)
				{
					...
					{
						for (int y = 0; y < highCardGame.getHand(0)
								.getNumOfCards(); y++)
						{
							if (ascHand[x].toString().equals(
									highCardGame.getHand(0).inspectCard(y).toString()))
							{
								return y;
							}
						}
				...
				{
					if (CardModel.Card
							.getSortRanking(ascHand[x]) == opponentCardRank)
					{
						for (int y = 0; y < highCardGame.getHand(0)
								.getNumOfCards(); y++)
						{
		...
		private static void computerPlayCard()
		{
			...
			System.out.println("Optimal card: " + card.toString() + " -> rank "
					+ CardModel.Card.getSortRanking(card));
		...
		private static void checkWinner()
		{
			...
			if (CardModel.Card.getSortRanking(cardPlayed[0]) > CardModel.Card
			...
			public void newGame()
			{
				...
				for (k = 0; k < numPacks; k++)
					for (j = 0; j < numJokersPerPack; j++)
						deck.addCard(new CardModel.Card('X',
								CardModel.Card.Suit.values()[j]));
	...
	static class CardModel
	{
		...
		static class Card
		{
			...
			public static final char[] valueRanks = { 'A', '2', '3', '4', '5', '6',
					'7', '8', '9', 'T', 'J', 'Q', 'K', 'X' };
